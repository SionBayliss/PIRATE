...
while ( <LOCI> ){
	...
	if( $curr_group ne $group ) {
		...
		if ( $paralog == 1 ) {
			...
			$split_total += $core_alleles;
			$split_no++ if $core_alleles > 0; 
...
# sort output loci list into group/alleles - maybe unnecessary #####
#`sort -k2,2 -k3,3 -k4,4 $output_dir/split_paralog_loci.tab > $output_dir/split_paralog_loci.tab.temp`;
#`mv $output_dir/split_paralog_loci.tab.temp $output_dir/split_paralog_loci.tab`;

# feedback
my $add_splits = $split_total - $paralog_check;
print " - $paralog_check of $no_paralog_families paralog families found in loci list.\n";
print " - $split_no families split into $add_splits additional core/accessory alleles - $split_total total\n";

# tidy up temporary files.
unlink "$output_dir/$loci_list.sorted";
